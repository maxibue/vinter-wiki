---
title: Vanilla Server Setup
description: 'Guide for setting up a Longvinter server on Linux.'
icon: 'server'
---

<Note>If you have any trouble following this guide, please send us a message on [Discord](https://discord.gg/longvinter) or the [Uuvana Forums](https://forum.uuvana.com/t/longvinter-questions-and-help). We are more than happy to help you out!</Note>

## Requirements and Pre-requisites

- SteamCMD installed in your system
- Broadband internet connection
- Min. 2 GB RAM
- Min. 64-bit Linux Operating System
- Router with the ability to port forward 

<Warning>Some ISPs might not allow you to port forward. In such case, we recommend you to purchase VPS / Dedicated Server hosting services.</Warning>

--- 
<Steps>
    <Step title="System Setup" icon="user-plus">

    ### Creating user and group to run the server

    ```shell
    sudo useradd -m -d /home/steam steamcmd
    sudo passwd steamcmd
    sudo usermod -aG sudo steamcmd
    sudo su steamcmd
    ```

    ### Installing GIT, GIT Large file system and other requisites

    <Tabs>
    <Tab title="Ubuntu/Debian">
        ```shell
        sudo apt update
        sudo apt install steamcmd
        sudo add-apt-repository multiverse
        sudo dpkg --add-architecture i386
        sudo apt update
        sudo apt install lib32gcc-s1 steamcmd
        ```
    </Tab>
    <Tab title="Arch-Linux">
        ```shell
        sudo pacman -Sy
        git clone https://aur.archlinux.org/steamcmd.git
        cd steamcmd
        makepkg -si
        sudo ln -s /usr/games/steamcmd /home/steam/steamcmd
        ```
    </Tab>
    <Tab title="Fedora">
        ```shell
        sudo yum update
        sudo yum install steamcmd
        ```
    </Tab>
    </Tabs>

   ### Making directory for Longvinter server
    ```shell
    sudo mkdir longvinter-server
    ```
    </Step>

<Step title="Install Steam SDK" icon="steam">
```shell
cd ~/
mkdir steamcmd-source
cd steamcmd-source
wget https://steamcdn-a.akamaihd.net/client/installer/steamcmd_linux.tar.gz
tar -xvzf steamcmd_linux.tar.gz
./steamcmd.sh +force_install_dir . +login anonymous +app_update 1007 +quit
```

### Copying Steam SDK to the right place

```shell
cd ~/.steam
mkdir sdk64
cp ~/steamcmd-source/linux64/steamclient.so ~/.steam/sdk64/
```

</Step>

<Step title="Configure Port-forwarding and Firewalls" icon="block-brick-fire">

<Note>**Warning:** In other tutorials it is asked to open the TCP Port 7777, do not do it. Unreal Engine doesn't use TCP connections - you would be leaving an unused port open by doing so!</Note>

<Tabs>
  <Tab title="IPTables">
    ```shell
    sudo iptables -I INPUT -p udp --dport 7777 -j ACCEPT
    sudo iptables -I INPUT -p udp --dport 27016 -j ACCEPT
    sudo iptables -I INPUT -p tcp --dport 27016 -j ACCEPT
    sudo iptables -I INPUT -p udp --dport 27015 -j ACCEPT
    sudo iptables -I INPUT -p tcp --dport 27015 -j ACCEPT
    ```
  </Tab>
  <Tab title="Uncomplicated Firewall (UFW)">
    ```shell
    sudo ufw allow 7777/udp
    sudo ufw allow 27016
    sudo ufw allow 27015
    ```
  </Tab>
</Tabs>

</Step>

<Step title="Install the Server" icon="download">

```shell
cd ~/
git clone https://github.com/Uuvana-Studios/longvinter-linux-server.git
sudo chmod -R ugo+rwx longvinter-linux-server/
```

</Step>

<Step title="Customize the Server" icon="cog">

```shell
nano ~/longvinter-linux-server/Longvinter/Saved/Config/LinuxServer/Game.ini
```

Add the following content:

```ini
[/Game/Blueprints/Server/GI_AdvancedSessions.GI_AdvancedSessions_C]
ServerName=Unnamed Island
ServerTag=Default
MaxPlayers=32
ServerMOTD=Welcome to Longvinter Island!
Password=
CommunityWebsite=www.longvinter.com

[/Game/Blueprints/Server/GM_Longvinter.GM_Longvinter_C]
AdminSteamID=76561198965966997
PVP=true
TentDecay=true
MaxTents=2
ChestRespawnTime=600
```

</Step>

<Step title="Run the Server" icon="play">

### Start the server manually

```shell
sh /home/steam/longvinter-linux-server/LongvinterServer.sh
```

### Start the server automatically with Systemd (Recommended)

```shell
sudo cp /home/steam/longvinter-linux-server/longvinter.service /etc/systemd/system/longvinter.service
sudo cp /home/steam/longvinter-linux-server/longvinter.socket /etc/systemd/system/longvinter.socket
sudo systemctl daemon-reload
```

</Step>

<Step title="Server Maintenance" icon="tools">

### Updating the server

```shell
bash /home/steam/longvinter-linux-server/LongvinterUpdate.sh
```

### Backing up your saves

```shell
bash /home/steam/longvinter-linux-server/LongvinterBackup.sh
```

</Step>

</Steps>